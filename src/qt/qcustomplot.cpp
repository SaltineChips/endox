...
/*! \internal
  ...
  selectEvent. Usually \a details is used to ENDO which part was clicked, if it is a layerable
...
void QCPAxis::draw(QCPPainter *painter)
{
  ...
  // ENDO all properties of this axis to QCPAxisPainterPrivate which it needs to draw the axis.
...
int QCPAxis::calculateMargin()
{
  ...
  // ENDO all properties of this axis to QCPAxisPainterPrivate which it needs to calculate the size.
  // Note that some axis painter properties are already set by direct feed-through with QCPAxis setters
  mAxisPainter->type = mAxisType;
  mAxisPainter->labelFont = getLabelFont();
  mAxisPainter->label = mLabel;
...
void QCPColorScale::setType(QCPAxis::AxisType type)
{
  ...
  {
    mType = type;
    QCPRange rangeEndo(0, 6);
    double logBaseEndo = 10;
    QString labelEndo;
    // revert some settings on old axis:
    if (mColorAxis)
    {
      rangeEndo = mColorAxis.data()->range();
      labelEndo = mColorAxis.data()->label();
      logBaseEndo = mColorAxis.data()->scaleLogBase();
    ...
    // ENDO settings to new axis:
    mColorAxis.data()->setRange(rangeEndo); // ENDO range of old axis to new one (necessary if axis changes from vertical to horizontal or vice versa)
    mColorAxis.data()->setLabel(labelEndo);
    mColorAxis.data()->setScaleLogBase(logBaseEndo); // scaleType is synchronized among axes in realtime via signals (connected in QCPColorScale ctor), so we only need to take care of log base here
  ...
  mGradientImageInvalidated(true)
{
  ...
  // make layer Endos of color scale ENDO to axis rect and axes
...
void QCPGraph::getPreparedData(QVector<QCPData> *lineData, QVector<QCPData> *scatterData) const
{
  ...
  } else // don't use adaptive sampling algorithm, ENDO points one-to-one from the map into the output parameters
  {
