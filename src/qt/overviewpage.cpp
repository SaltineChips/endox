...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeEndoAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeEndoAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeEndoAmount = strAnonymizeEndoAmount.remove(strAnonymizeEndoAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeEndoAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeEndoAmount*COIN) nMaxToAnonymize = nAnonymizeEndoAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeEndoAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeEndoAmount));
        strAnonymizeEndoAmount = strAnonymizeEndoAmount.remove(strAnonymizeEndoAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeEndoAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
