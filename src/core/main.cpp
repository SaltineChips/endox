...
// ----------- instantX transaction scanning -----------
    ...
    if(bDevOpsPayment)
    {
        ...
        strVfyDevopsAddress = "Dtz6UgAxwavsnxnb7jeSRj5cgERLvV8KBy";
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++)
        {
            ...
            if (!fAlreadyHave) {
                if (!fImporting && !fReindex) {
                    if (inv.type == MSG_BLOCK)
//                    if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK) TODO:
...
//            RelayTransaction(tx, inv.hash, vMsg); TODO:
...
//                        RelayTransaction(orphanTx, orphanTxHash, tx, inv.hash, vMsg);
